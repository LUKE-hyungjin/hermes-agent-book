...
> 이 장은 터미널에서 직접 실행하는 `hermes` 명령을 정리한 참조 문서입니다. 대화 중 `/model` 같은 슬래시 명령이 아니라, 셸에서 치는 실제 CLI 엔트리포인트를 다룹니다.
...
이 문서를 레퍼런스로 쓸 때는 "명령 이름을 찾고", 세부 사용감은 [05. CLI 인터페이스](#05-cli)에서 보완하면 좋습니다. Hermes CLI는 기능이 많지만 구조는 단순합니다. 전역 옵션으로 실행 방식을 정하고, 하위 명령으로 기능 영역을 고른다고 생각하면 대부분의 명령이 빠르게 정리됩니다.

---

**이전:** [← 29. API 서버](#29-api-server)
**다음:** [31. 환경 변수 레퍼런스 →](#31-env-vars)
