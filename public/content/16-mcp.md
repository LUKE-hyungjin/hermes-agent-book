...
> MCP는 Hermes 바깥의 도구 서버를 안전하게 연결하는 확장 계층입니다.

:::tip
이 장은 초보자가 첫날 바로 설정해야 하는 기능은 아닙니다. 먼저 Hermes 설치와 기본 대화가 익숙해진 뒤, 외부 도구를 붙이고 싶을 때 돌아와도 충분합니다.
:::
...
## 0.6.0의 큰 변화: MCP 서버 모드

0.6.0부터는 Hermes가 MCP 클라이언트일 뿐 아니라 **MCP 서버**로도 동작할 수 있습니다. 즉, Claude Desktop, Cursor, VS Code 같은 MCP 호환 클라이언트가 Hermes의 대화·세션·검색 기능에 붙는 구조가 가능해졌습니다.

```bash
hermes mcp serve
```

이 모드는 "Hermes가 외부 MCP 서버에 붙는 기능"과 반대 방향입니다.

- 기존 MCP 장의 앞부분: Hermes → 외부 MCP 서버 연결
- 새 MCP 서버 모드: 외부 MCP 클라이언트 → Hermes 연결

초보자는 당장 이 기능이 필요 없을 수 있지만, 이미 Cursor나 Claude Desktop 같은 툴을 쓰고 있다면 0.6.0의 가장 큰 변화 중 하나로 봐도 됩니다.
